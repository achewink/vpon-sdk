...
!function(a) {
    a(function() {
        ...
        var buildNav = function(nav_ul) {
            ...
            var h1_list = $('.bs-docs-container h1');
            // var h1_ids = [];
            // h1_list.each(function(){ h1_ids.push(this.id); });
            var h1_last = h1_list.length - 1;

            h1_list.each(function(i1, h1_item) {
                h1_item = $(h1_item);

                var h1_nav = navItem(h1_item);

                var h2_list;
                if (i1 != h1_last) {
                    h2_list = h1_item.nextUntil('h1', 'h2');
                    if (!h2_list.length) {
                        h2_list = h1_item.nextUntil('h1', 'h3');
                    }
                } else {
                    h2_list = h1_item.nextAll('h2');
                    if (!h2_list.length) {
                        h2_list = h1_item.nextAll('h3');
                    }
                }
                if (h2_list.length > 0) {
                    var h2_ul = $('<ul>').addClass('nav');
                    h2_list.each(function(i2, h2_item) {
                        h2_ul.append(navItem($(h2_item)));
                    });
                    h1_nav.append(h2_ul);
                }
                nav_ul.append(h1_nav);
                h1_nav.hide();
                h1_nav.fadeIn();
            });
        };
        ...
        d.scrollspy({target: ".bs-sidebar"}), c.on("load", function() {
            ...
            a(".bs-top").affix()
        }, 100), a(".toolt ip-demo").tooltip({selector: "[data-toggle=tooltip]",container: "body"}), a(".tooltip-test").tooltip(), a(".popover-test").popover(), a(".bs-docs-navbar").tooltip({selector: "a[data-toggle=tooltip]",container: ".bs-docs-navbar .nav"}), a("[data-toggle=popover]").popover(), a("#loading-example-btn").click(function() {
            var b = a(this);
    ...
    $('img').attr({style:'margin: 13px 5px 45px 0px; box-shadow: 0 3px 6px rgba(0,0,0,0.16), 0 3px 6px rgba(0,0,0,0.23)', class:'img-responsive' });
    $('.btn').attr({style:'margin: 10px 10px 10px 10px;'});
}(jQuery);
